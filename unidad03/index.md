---
---

[Mi Portfolio](../) -> [Unidad 03](./)

# Unidad 03 — Sistemas Cloud

En esta formación se han trabajado las tecnologías clave de los sistemas Cloud, abordando las características principales de Amazon Web Services (AWS), Google Cloud Platform (GCP) y Microsoft Azure.

Se han documentado los retos y prácticas realizados en esta unidad.

---

## Retos

### 1. Inicialización de Entornos Cloud

Configuración inicial de los entornos AWS y GCP, gestión de costes,
Free Tier y medidas básicas de seguridad.

[Ver Reto 1 →](./reto01_registro/)

---

### 2. Despliegue y Configuración de Instancia EC2 en AWS

Despliegue de una instancia EC2, configuración del acceso SSH,
actualización del sistema y primeras medidas de seguridad.

[Ver Reto 2 →](./reto02_instanciaEC2/)

---

### 3. Almacenamiento y recuperación de archivos

Creación de buckets de almacenamiento, subida de archivos,
configuración de permisos y generación de URLs seguras y temporales.

[Ver Reto 3 →](./reto03_storage/)

---

### 4. Despliegue de máquinas virtuales en GCP y Azure

Despliegue y configuración de máquinas virtuales en servicios cloud,
realizando conexiones remotas y explorando las principales opciones
de administración, seguridad, almacenamiento, red, monitorización
y automatización.

[Ver Reto 4 →](./reto04_VM/)
